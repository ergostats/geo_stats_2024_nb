...
zonas <- st_read("../data/GEODATABASE_NACIONAL_2021/GEODATABASE_NACIONAL_2021.gdb/", layer = -----)

dmq_zonas <- zonas %>% filter(str_detect(zon, -----))
```
...
ggplot() +
  geom_sf(data = -----, color = "darkgrey", size = 0.5) +
  geom_sf(data = ----, aes(color = ------), size = 1, alpha = 0.6) +
  scale_color_brewer(-----) +
  theme_minimal() +
...
contar_amenidades <- amenidades %>% 
  select(------(names(dmq_zonas))) %>%
  st_join(dmq_zonas, join = -----) 
...
-----(mapa_calor_salud, mapa_calor_educacion, -----)
```
...
data_modelo <- contar_amenidades %>% 
  -----(names_from = categoria, values_from = n_amen)
```
...
data_modelo <- data_modelo %>% 
  ------(censo_hogar_s, by = "zon")
```

```{r}
# Rellenar valores faltantes con ceros
data_modelo <- data_modelo %>% mutate(----(educacion:n_hijos, ~replace_na(.x, 0)))
```
...
modelo <- ----(n_hijos ~ salud + educacion, data = data_modelo)
...
vecinos_pesos <- ----(vecinos, style = "W", zero.policy = TRUE)
```
...
dmq_zonas <- -------(dmq_zonas, data_modelo, by = "zon") %>% -------

# Calcular índice de Moran
moran <- ------(dmq_zonas$n_hijos, vecinos_pesos, alternative = "greater")
...
lm_moran_test <- -------(modelo, vecinos_pesos)
